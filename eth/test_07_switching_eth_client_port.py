...
class SwitchingEthClientPortRoot(BaseCase):
    @classmethod
    def setup_class(cls):
        cls.attempts = 3
        with cls.SafeSetup(SwitchingEthClientPortRoot, cls):
    ...
    def teardown_class(cls):
        with cls.SafeTeardown(SwitchingEthClientPortRoot, cls):
    ...
    @allure.title("Switch port eth clients on leaf")
    def test_02_switch_port_eth_client_on_leaf(self):
        log.info(f"Switch port eth client {self.attempts} times")
        # default port name
        first_port = "eth0"
        second_port = "eth1"
        for i in range(0, self.attempts):
            log.info("")
            log.info(f"Switch port eth client. Attempt: {i + 1}")
            self.switch.api.disconnect_eth_client(self.leaf_name, self.eth1_name)
            self.switch.api.disconnect_eth_client(self.leaf_name, self.eth2_name)
            log.info("Check loop status before connect eth client to node")
            self.pod.leaf.wait_eth_connection_ready()
            log.info(f"Connect {self.eth1_name} client to {self.leaf_name} device on {first_port}")
            self.switch.api.connect_eth_client(self.leaf_name, self.eth1_name, connect_port=first_port)
            log.info(f"Connect {self.eth2_name} client to {self.leaf_name} device on {second_port}")
            self.switch.api.connect_eth_client(self.leaf_name, self.eth2_name, connect_port=second_port)
            first_port, second_port = self.get_port(first_port, second_port)

            log.info("Check sanity on leaf client")
            assert self.pod.leaf.poll_pod_sanity() == 0, f"Sanity failed on {self.leaf_name}"
    ...
    @staticmethod
    def get_port(first_port, second_port):
        if first_port == "eth0":
            first_port = "eth1"
        elif first_port == "eth1":
            first_port = "eth0"

        if second_port == "eth0":
            second_port = "eth1"
        elif second_port == "eth1":
            second_port = "eth0"
        return first_port, second_port


@pytest.mark.network_mode(target="router")
@pytest.mark.duration(seconds=807)
@pytestrail.case("C270134", "C396944", "C566041")
@allure.title("Switching eth client port router")
class Test07SwitchingEthClientPortRouter(SwitchingEthClientPortRoot):
    pass


@pytest.mark.network_mode(target="bridge")
@pytest.mark.duration(seconds=809)
@pytestrail.case("C270133", "C396903", "C566040")
@allure.title("Switching eth client port bridge")
class Test07SwitchingEthClientPortBridge(SwitchingEthClientPortRoot):
    pass
